...
# =========================
try:
    # fg_img = cv2.imread("Dataset/adip_final_dataset/shoes.png")
    ...
    fg_img = cv2.imread("Dataset/adip_final_dataset/guy.jpg")
    bg_img = cv2.imread("Dataset/adip_final_dataset/family.jpg")
    if fg_img is None:
...
def on_fg_click(event, x, y, flags, param):
    ...
    if event == cv2.EVENT_LBUTTONDOWN:
        ...
        if w > 10 and h > 10:
            save_state() # <--- SAVES STATE BEFORE APPLYING NEW MASK
            
            if not state["initialized"]:
                state["fg_mask"] = np.zeros(fg_img.shape[:2], np.uint8)
                cv2.grabCut(fg_img, state["fg_mask"], final_rect, 
                            state["bgdModel"], state["fgdModel"], 
                            7, cv2.GC_INIT_WITH_RECT)
                state["initialized"] = True
                print(">>> Initial Cut Done. Draw more boxes to add parts.", flush=True)
            else:
                # Mark the new rectangle as 'Probable Foreground'
                ...
                # Only set pixels to GC_PR_FGD (Probable) if they are NOT ALREADY GC_FGD (Locked/Green)
                # We use np.where to preserve the Green pixels
                state["fg_mask"][ry:ry2, rx:rx2] = np.where(
                    roi == cv2.GC_FGD,  # If currently Green (Locked)
                    cv2.GC_FGD,         # Keep it Green
                    cv2.GC_PR_FGD       # Else, make it Pink (Probable)
                )
                
                # Re-run GrabCut to refine
                cv2.grabCut(fg_img, state["fg_mask"], None, 
                            state["bgdModel"], state["fgdModel"], 
                            5, cv2.GC_INIT_WITH_MASK)
                print(">>> Mask Updated. Press SPACE to confirm/lock if it looks good.", flush=True)
